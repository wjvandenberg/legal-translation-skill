...
def _collect_required_tokens_with_options(paragraph_entry, post_spacing_fix,
    ...
    en_segments as-is and tokenise — preserves rev41 behaviour byte-for-byte.
    ...
    applied tokenise identically across the gate. TWO ROUTES REACH THAT MIRROR
    and the first is preferred:
      ...
      * `spacing_record` None — fall back to PREDICTING with
        `will_fix_spacing_fire`, the rev42 behaviour, for any caller with no
        journal to read.
...
def validate(paragraphs_json_path, document_xml_path, post_spacing_fix=False):
    ...
    # tokens match the raw post-distribute applied text — rev41 behaviour.
    #
