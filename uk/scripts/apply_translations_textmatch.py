...
def distribute_text_across_elements(elements, text, preserve_source_boundary_whitespace=True,
    ...
    whitespace from EITHER side. The caller knows what its neighbours carry and this function
    ...
    neighbouring segment to consult.

    WHAT IS DELIBERATELY NOT CHANGED: the repair itself. A source edge carrying one space,
    with an English that dropped it and no neighbour supplying it, is still restored -- that
...
def apply_trackchanges_inplace(orig_p, en_text, it_text,
    ...
    if en_segments:
        ...
        if en_types == xml_types:
            ...
            #
            # `None` -> the source stays, so the source text is what the neighbour will see;
            # `''`   -> the segment is cleared and is NOT a neighbour at all, which is why
            ...
            def _boundary_ws(i, back):
                """Does the nearest non-empty neighbour already supply whitespace here?"""
            ...
            for seg_i, (en_seg, xml_seg) in enumerate(zip(en_segments, merged_xml)):
                ...
                # behaviour to a device that uses the empty string
...
def textmatch_apply(orig_docx_path, paragraphs_json_path, output_xml_path,
    ...
    for entry in entries_sorted:
        ...
        # a 3-character clause number shortens `en` by 30 and every authored offset after the
