...
for stem in args.fixture:
    ...
    (outdir / "READ-ME.txt").write_bytes(("\n".join([
        ...
        "WHAT THIS BRANCH CHANGED — BRANCH 8, EXTRACTION COMPLETENESS (C28, C12, M1). AND",
        "THE HONEST ANSWER IS: NOTHING THESE PAGES CAN SHOW, WHICH IS WHY IT IS SAID FIRST.",
        "It adds a READ-ONLY mode to validate_apply.py and rewrites one step document. It",
        "writes no document, changes no delivered byte, and does not touch apply. So `old`",
        "and `new` here are the SAME APPLY CODE and an all-quiet render is the EXPECTED",
        "result — the only thing it evidences is that nothing else broke.",
        "",
        "DO NOT HUNT THESE PAGES FOR THIS BRANCH'S WORK. It is not on them, in either arm,",
        "and could not be: a check that reports what a capture MISSED has no page, because",
        "the thing it reports is an ABSENCE from a JSON file, not a mark on a document.",
        "Manufacturing a visual arm for it would be theatre — measured twice already on this",
        "project, where a positive control proved LibreOffice renders no glossary",
        "placeholder text at all, and where neither of A19's two surfaces renders anywhere.",
        "",
        "  WHERE THIS BRANCH'S PROOF ACTUALLY IS, and both halves are in bytes:",
        "    tests/test_extraction_completeness.py   32 checks on each variant, RED FIRST on",
        "      four negative inputs — a paragraph deleted from the capture, a paragraph",
        "      truncated at its first w:t, a capture stripped of the newline extraction emits",
        "      at a plain w:br, and a document with no text at all (which must be VOID, not",
        "      clean). One of those four caught a real defect IN THE CHECK.",
        "    tools/extraction_corpus_arm.py          the corpus arm. ZERO uncaptured body",
        "      paragraphs on all 12 reachable frozen intermediates, D06 declared VOID for",
        "      want of a readable source, and a PLANTED TRUNCATION FIRING as the positive",
        "      control — so the zeros are calibration rather than silence. Its auxiliary arm",
        "      reports 40 paragraphs across 6 documents that no capture accounts for.",
        "",
        "  AND ONE SHAPE HAD NO FIXTURE IN THIS DIRECTORY AT ALL BEFORE NOW: not one",
        "  contained a `<w:br/>`. extract_paragraphs emits a newline at a PLAIN break and",
        "  nothing at a PAGE break, a contract stated in a comment and asserted by nothing —",
        "  and the first version of the completeness check ignored it and reported FOUR real",
        "  corpus documents as damaged. It had reproduced its own reader as a finding.",
        "  extraction-completeness.docx now carries both breaks, adjacent, so a reader that",
        "  treats them alike fails one of them whichever way it is wrong.",
        "",
        "PREVIOUS SLICE, FOR ORIENTATION ONLY — A19, translatable text held in GRAPHIC",
