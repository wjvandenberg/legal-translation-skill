...
for stem in args.fixture:
    ...
    (outdir / "READ-ME.txt").write_bytes(("\n".join([
        ...
        "WHAT THIS BRANCH CHANGED — A19, translatable text held in GRAPHIC METADATA. It",
        "changes translate_headers_footers.py and extract_paragraphs.py; it does NOT touch",
        "apply. So on these pages the `old` and `new` arms are the SAME APPLY CODE, and an",
        "all-quiet render is the expected result rather than evidence of anything. The line",
        "above about the baseline arm says so too when it fires.",
        "",
        "AND NEITHER OF A19's SURFACES HAS A PAGE IN ANY RENDERER — NOT THIS ONE, NOT WORD.",
        "This is not a limitation to work around; it is what the finding IS. Alt text is an",
        "XML ATTRIBUTE that a SCREEN READER SPEAKS: it is never painted, in any viewer, by",
        "design. A chart title lives in its own part. So a three-arm visual diff of A19",
        "would be theatre, and manufacturing one would be worse than having none — measured",
        "on the previous slice, where a positive control proved LibreOffice renders no",
        "glossary placeholder text at all and a translated and an untranslated part gave",
        "byte-IDENTICAL page text. DO NOT HUNT THESE PAGES FOR THE ALT TEXT. It is not on",
        "them, in either arm, and never could be.",
        "",
        "  IT IS ALSO MOST OF WHY THE DEFECT SURVIVED. A surface no page shows is invisible",
        "  to every instrument that looks at a page, which is every instrument a reviewer",
        "  naturally reaches for. Until 2026-09-09 nothing in either shipped tree mentioned",
        "  it: `docPr`, `SmartArt`, `w:drawing`, `a:graphic`, `c:title`, `wp:inline` and",
        "  `diagrams` measured ZERO across all 396 files of both trees, control firing.",
        "",
        "  THE PROOF OF THIS SLICE IS IN BYTES, and it is tests/test_graphic_metadata.py:",
        "  95 checks on each variant. The source-language @descr, @title and v:shape/@alt",
        "  are GONE from the written header and footer and the English is in their place;",
        "  @name is untouched, Word having assigned it; the drawing and the VML shape",
        "  survive structurally; and a scaffold entry with no `kind` key is still a",
        "  paragraph entry. tools/hf_corpus_diff.py is the corpus arm — 3 documents",
        "  byte-identical old-vs-new, 0 graphic surfaces corpus-wide, control FIRED.",
        "",
        "  graphic-metadata.docx IS NOT RENDERED BY THIS TOOL and that is deliberate: every",
        "  one of its six shapes is either invisible on a page or already covered by",
        "  containers.docx. Render containers.docx, which is what --fixture containers does.",
        "",
        "PREVIOUS SLICE, FOR ORIENTATION ONLY — C19, the glossary part's route in and out.",
        ...
        "",
        "THE SLICE BEFORE THAT, FOR ORIENTATION ONLY — A16 and N1, the container inventory: a",
