...
for stem in args.fixture:
    ...
    (outdir / "READ-ME.txt").write_bytes(("\n".join([
        ...
        "",
        # THIS BLOCK IS REWRITTEN ON EVERY BRANCH, NEVER APPENDED TO, AND IT HAS NOW HAD TO
        # BE ON EACH OF THE LAST EIGHT. Nothing checks it. Left describing the previous
        # change it sends a reviewer hunting for something that is already in the `old` arm,
        # and a reviewer who cannot find what the text promises reads a correct page as a
        # defect -- which happened on the table-of-contents slice, where two deliberate
        # refusals were read as damage because nothing on the page said they were deliberate.
        #
        # EIGHT CONSECUTIVE REWRITES IS NO LONGER A CADENCE, IT IS A DESIGN FACT: this text
        ...
        "WHAT THIS BRANCH CHANGED — BRANCH 10 SLICE 1, THE FORMATTING RECORD (B1, B7).",
        "AND THE HONEST ANSWER IS AGAIN: NOTHING THESE PAGES CAN SHOW, WHICH IS WHY IT IS",
        "SAID FIRST — AND WHY IT WILL STOP BEING TRUE AT THE NEXT SLICE.",
        "post_process.py's journal already recorded every piece of TEXT it changed. It was",
        "DECLARED blind to formatting, and reported that blindness as a bare count. It now",
        "records formatting too: every run whose properties moved — an italic stripped —",
        "and every paragraph whose properties moved — a page break imposed — on the SAME",
        "numbering the text record uses. It changes NO delivered byte; that is the slice's",
        "acceptance condition, stated before it was measured, not an incidental property.",
        "It does not touch apply at all, so `old` and `new` here are the SAME APPLY CODE",
        "and an all-quiet render is the EXPECTED result.",
        "",
        "AND READ THIS BEFORE THE NEXT SLICE'S PAGES: BRANCH 10 IS THE BRANCH WHERE THAT",
        "STOPS HOLDING. Slices 2, 3 and 4 make the passes CONDITIONAL, so they change",
        "delivered bytes on every document and their acceptance is the OPPOSITE of this",
        "one's — the bytes MUST move, and every movement must be explained by a register",
        "row. On those slices these pages carry the branch's work and are worth hunting.",
        "",
        "DO NOT HUNT THESE PAGES FOR THIS BRANCH'S WORK. A branch whose whole purpose is",
        "that the document does NOT change cannot have a page: there is no `before` and",
        "`after` to compare, by construction. If you found a difference here, THAT would be",
        "the defect. Manufacturing a visual arm would be theatre — measured twice already on",
        "this project, where a positive control proved LibreOffice renders no glossary",
        "placeholder text at all, and where neither of A19's two surfaces renders anywhere.",
        "",
        "  WHERE THIS SLICE'S PROOF ACTUALLY IS, and all of it is in bytes:",
        "    tests/test_change_journal.py            53 checks on each variant, 0 failures,",
        "      0 void. RED FIRST, and the red is worth reading rather than taking on trust:",
        "      run against a post_process with no formatting record it reported 8 failures,",
        "      naming UNACCOUNTED ORDINAL 0 — the italic run — and UNACCOUNTED PARAGRAPH 1",
        "      — the imposed page break. That is the blind spot branch 9 declared, made to",
        "      fail on demand. Its positive control drops one formatting record and requires",
        "      the replay to report that run as unaccounted.",
        "    tools/postprocess_corpus_arm.py         the corpus arm, and it exists because",
        "      NOTHING IN THIS REPOSITORY RAN post_process OVER A REAL DOCUMENT BEFORE IT.",
        "      13 of 13 frozen workdirs examined, 0 not examined, every one byte-identical",
        "      to the pinned baseline — which is how `additive` is proved rather than",
        "      asserted. 3 documents had text to account for under uk and 7 under us; all",
        "      accounted; the rest are CALIBRATION and the tool says so rather than letting",
        "      a population of zeros read as a population of passes. THIS SLICE ADDED A",
        "      FORMATTING ARM TO IT, with its own positive control in a DIFFERENT host",
        "      document — the one whose formatting moves most is not the one whose text",
        "      moves most, and a control planted in the wrong document proves the arm can",
        "      fail somewhere it was never going to be asked. 12 of 13 measured at BOTH",
        "      levels; the 13th carries a DECLARED limitation, named in the output: the",
        "      strip pass deletes elements, so every ordinal after it shifts and no record",
        "      written against them would be true. The PARAGRAPH level still answers there,",
        "      and the first version of that check voided both levels together — which the",
        "      CORPUS caught and no fixture could have.",
        "",
        "  AND THE COUNT BRANCH 9 COULD ONLY PRINT IS NOW AN ACCOUNT. On the one frozen",
        "  document with a pre-post_process snapshot, branch 9 could say `39 fixes changed",
        "  no text` and nothing more — no place, no pass. This slice resolves that 39 into",
        "  36 runs losing their italic and 3 paragraphs gaining a page break, each located",
        "  and each naming its pass, read by a SECOND reader that imports nothing from the",
        "  script it measures. The 3 matches register B7's measured `pageBreakBefore` 0 -> 3",
        "  on that document exactly. THE 36 DOES NOT MATCH B1's 38, and that gap is recorded",
        "  in the row rather than smoothed: the pass's own count, the element delta and the",
        "  located records all say 36, three independent ways, on the CURRENT tree.",
        "",
        "  A SECOND DOCUMENT IS THE QUIETER RESULT AND THE MORE GENERAL ONE: one frozen",
        "  workdir reported TOTAL 2 fixes against 0 journalled edits and a self-check of",
        "  `accounted` — all correct under a TEXT contract, and together reading as `this",
        "  stage changed nothing` when it had changed formatting. Those 2 are now located as",
        "  1 run and 1 paragraph. That is the reading this slice exists to make impossible.",
        "",
        ...
        "baseline, so it is no longer visible as a difference here. It had no page either —",
        "what it reports is an ABSENCE from a JSON file, not a mark on a document.",
        "",
        "THE SLICE BEFORE THAT, FOR ORIENTATION ONLY — A19, translatable text held in GRAPHIC",
        "METADATA. That is now in the pinned baseline, so it is no longer visible as a",
        "difference here. Neither of its surfaces ever had a page in any renderer: alt text",
        "is an XML attribute a SCREEN READER SPEAKS, never painted in any viewer, and a",
        "chart title lives in its own part — which is most of why the defect survived, a",
        "surface no page shows being invisible to every instrument a reviewer reaches for.",
        "Its proof is tests/test_graphic_metadata.py, 95 checks per variant, and",
        "graphic-metadata.docx is deliberately NOT rendered by this tool.",
        "",
        "THE SLICE BEFORE THAT, FOR ORIENTATION ONLY — C19, the glossary part's route in and out.",
        "That is now in the pinned baseline, so it is no longer visible as a difference",
        "here. glossary.docx labels its own three rows; its proof was never visual either.",
        "",
        "AND THE ONE BEFORE THAT, FOR ORIENTATION ONLY — A16 and N1, the container inventory: a",
        ...
        "  chart title lives in its own part. SLICE 3 HAS NOW CLOSED A19, and apply must",
        "  still leave both alone — the translation route is Step 8b's, which writes header",
        "  and footer parts only, so this fixture's BODY alt text is reported and NOT",
        "  translated. test_container_inventory.py asserts apply's hands-off in both",
        "  directions; test_graphic_metadata.py owns the fix.",
        "",
