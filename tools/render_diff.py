...
for stem in args.fixture:
    ...
    (outdir / "READ-ME.txt").write_bytes(("\n".join([
        ...
        "WHAT THIS BRANCH CHANGED — BRANCH 9, THE CHANGE JOURNAL (C15, B6). AND THE HONEST",
        "ANSWER IS AGAIN: NOTHING THESE PAGES CAN SHOW, WHICH IS WHY IT IS SAID FIRST.",
        "post_process.py now writes a record of every piece of text it changed and which",
        "pass changed it. It changes NO delivered byte — that is the branch's acceptance",
        "condition, not an incidental property — and it does not touch apply at all. So",
        "`old` and `new` here are the SAME APPLY CODE and an all-quiet render is the",
        "EXPECTED result; the only thing it evidences is that nothing else broke.",
        "",
        ...
        "",
        "  WHERE THIS BRANCH'S PROOF ACTUALLY IS, and all of it is in bytes:",
        "    tests/test_change_journal.py            32 checks on each variant. RED FIRST —",
        "      it ran against a post_process that wrote no journal and failed for exactly",
        "      that reason. Its positive control removes one entry from a journal and",
        "      requires the completeness comparison to fail, AND IT CAUGHT TWO REAL DEFECTS",
        "      IN THE CHECK ITSELF on its first run: the element record was being walked but",
        "      not used, so deleting an entry changed nothing, and the entries carried no",
        "      paragraph id at all. Both read as working.",
        ...
        "      a population of zeros read as a population of passes.",
        "",
        "  AND THE CORPUS FOUND WHAT THE FIXTURES COULD NOT, which is the argument for the",
        "  arm existing at all. One frozen document reported TOTAL 2 fixes while the journal",
        "  recorded 0 edits and its self-check said `accounted` — every one of those correct",
        "  under a TEXT contract, and together reading as `this stage changed nothing`. It",
        "  had changed formatting. The journal now carries a `non_text_fixes` figure naming",
        "  any pass that made a change it does not describe, and on the one document with a",
        "  pre-postprocess snapshot that figure is 39 — a silent blind spot turned into a",
        "  number, on the same document class where the register measured 38 destroyed",
        "  italic spans.",
        "",
