...
11. Spelling violations against active variant (US default: flags UK spellings; --variant uk: flags US spellings)
12. Article vs Section (US default) / Clause (UK) for internal cross-references
...
US_SPELLINGS = [
    ...
    # Long-tail US→UK doublets flagged under --variant uk (added rev45b).
    # Limited to truly bidirectional doublets — see post_process.py
    # UK_SPELLING table for the rationale (excludes check, tire, curb, story,
    # while, among, learned, inquire, aging as false-friend or non-flip cases).
    (r'\bplow\b', 'plough'), (r'\bplows\b', 'ploughs'),
    (r'\bplowing\b', 'ploughing'), (r'\bplowings\b', 'ploughings'),
    (r'\bplowed\b', 'ploughed'),
    (r'\baluminum\b', 'aluminium'),
    (r'\bmaneuver\b', 'manoeuvre'), (r'\bmaneuvers\b', 'manoeuvres'),
    (r'\bmaneuvered\b', 'manoeuvred'), (r'\bmaneuvering\b', 'manoeuvring'),
    (r'\bmold\b', 'mould'), (r'\bmolds\b', 'moulds'), (r'\bmolded\b', 'moulded'),
    (r'\bskeptic\b', 'sceptic'), (r'\bskeptical\b', 'sceptical'),
...
# specific bank/institution names removed to avoid signaling.
OK_CONTEXTS = [
...
def check_us_spelling(root, verbose):
    """Check 11a: US spelling violations (used ONLY under --variant uk).
    The default variant for this skill is US English, so this check is
    inactive by default; it fires only when the operator has explicitly
    passed --variant uk."""
    issues = []
    for t in root.iter(f'{{{W}}}t'):
        if t.text:
...
# Spelling table used by default. The skill ships with --variant us as the
# hardcoded default, so this list is the one applied unless the user
# explicitly requests UK English (in which case the inverse check
# check_us_spelling above is the one that fires).
UK_SPELLINGS_INV = [
    ...
    # Long-tail UK→US doublets flagged under --variant us (default) — added
    # rev45b after the 'ploughing' post-mortem.
    (r'\bplough\b', 'plow'), (r'\bploughs\b', 'plows'),
    (r'\bploughing\b', 'plowing'), (r'\bploughings\b', 'plowings'),
    (r'\bploughed\b', 'plowed'),
    (r'\bwhilst\b', 'while'), (r'\bamongst\b', 'among'),
    (r'\blearnt\b', 'learned'), (r'\bspelt\b', 'spelled'),
    (r'\bdreamt\b', 'dreamed'), (r'\bburnt\b', 'burned'),
    (r'\bspoilt\b', 'spoiled'),
    (r'\baluminium\b', 'aluminum'),
    (r'\bmanoeuvre\b', 'maneuver'), (r'\bmanoeuvres\b', 'maneuvers'),
    (r'\bmanoeuvred\b', 'maneuvered'), (r'\bmanoeuvring\b', 'maneuvering'),
    (r'\bkerb\b', 'curb'), (r'\bkerbs\b', 'curbs'),
    (r'\btyre\b', 'tire'), (r'\btyres\b', 'tires'),
    (r'\bmould\b', 'mold'), (r'\bmoulds\b', 'molds'), (r'\bmoulded\b', 'molded'),
    (r'\bcheque\b', 'check'), (r'\bcheques\b', 'checks'),
    (r'\bstorey\b', 'story'), (r'\bstoreys\b', 'stories'),
    (r'\bsceptic\b', 'skeptic'), (r'\bsceptical\b', 'skeptical'),
    (r'\bscepticism\b', 'skepticism'),
    (r'\benquire\b', 'inquire'), (r'\benquiry\b', 'inquiry'),
...
def check(xml_path, verbose=False, source_json=None, variant='us',
    ...
    # Spelling check is variant-dependent (US is the hardcoded default).
    if variant == 'uk':
...
if __name__ == '__main__':
    ...
    ap = argparse.ArgumentParser(
        ...
        epilog=(
            "Note: --variant defaults to 'us'. Only pass --variant uk if the "
            "user's original prompt explicitly requested UK English.\n"
    ...
    ap.add_argument('--variant', choices=('uk', 'us'), default='us',
                    help='English variant for the spelling check (default: us).')
