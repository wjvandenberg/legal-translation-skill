...
def _is_external_article_ref(joined_text, match_end):
    ...
    **Rev16 — pure structural walk.** Instead of enumerating the
    ...
    # default (matches v5 behavior for bare "Article N.").
    return False
...
def _run_validate_apply_post_strip(xml_path, paragraphs_json_path):
    ...
    if result.returncode != 0:
        raise RuntimeError(
            ...
            f"WORK OUT WHICH SIDE IS WRONG BEFORE EDITING EITHER. The "
            f"document may have drifted, or paragraphs.json may be the "
            f"inaccurate record — this gate cannot tell you which. "
            ...
            f"the side that is wrong and re-run from Step 5 (apply). "
