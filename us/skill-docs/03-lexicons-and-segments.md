...
- `references/ip-it-technology.md` — Combined IP (patents, trademarks, copyright, licensing, R&D, technology transfer) and IT/SaaS (software licenses, cloud services, development agreements)
...
is almost always a **whole-word** replacement (e.g. "Section 12" → "Section 13"
under the US-default variant, "Clause 12" → "Clause 13" under `--variant uk`), not
...
1. `coalesce_fragmented_tcs.py` collapses character-level source edits into
   ...
   pair `Section 12 → Section 13` in English (US default; UK: `Clause 12 → Clause 13`),
   because character-level edits do not survive word-level translation.
...
counts, because Spanish conversion artifacts cluster on the insertion side).
...
- [ ] You did NOT skim the lexicons or summarize — you read them end-to-end
