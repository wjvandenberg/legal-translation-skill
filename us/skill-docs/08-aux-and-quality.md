...
version labels like `Versie DRAFT 1 – juli 2021`, initialing-box labels like
...
- **Translate every source-language token by default.** Agreement titles
  ...
  LUOTTAMUKSELLINEN → CONFIDENTIAL`), initialing-box labels
...
Recognized placeholder types include `<<PAGE>>`, `<<NUMPAGES>>`, `<<SECTIONPAGES>>`,
...
control that is currently empty (glossary). Grading: Step 8 outputs are penalized under Criterion 5
(Completeness), capping at 7 if any source-language token survives. Comments inside
`w:delText` are penalized under Criterion 13 (Track Changes Fidelity).
...
**Variant flag — use the same variant as Step 6.** `--variant us` is the hardcoded
default. Only pass `--variant uk` if Step 6 was run with `--variant uk`, which itself
requires that the user's original prompt explicitly requested UK English. Mismatched
