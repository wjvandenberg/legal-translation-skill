...
- Double punctuation (::, .., ,,, ;;)
- Terminology standardization (Facility Agreement, Secured Assets, etc.) — **never
...
* **A tracked-change wrapper holding only punctuation is removed only when it is a
  ...
  closing bracket and no period, and every gate reported PASS. The line to hold on to:
...
* **`Article N` is left alone where the sentence does not say which instrument it means.**
  ...
  citation into a section number that does not exist. Where the reference points backward —
