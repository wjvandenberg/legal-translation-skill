...
5. **Skill-gate semantics.** A gate firing produces a `SKILL GATE FIRED — INTENTIONAL BLOCK, NOT A SCRIPT ERROR` banner. This is the script doing its job, not the script breaking. **Do NOT work around a gate by patching the script or skipping the validator** — fix the input (usually paragraphs.json) and re-run.
   ...
   **5a. A check can be wrong IN SCOPE, and saying so is not permission to bypass it.** Everything in rule 5 stands: **never patch the installed skill**, never skip the validator, never pass an override "just this once". But a check can be correctly written and **wrongly scoped** — firing on input that is in fact a faithful translation of the source. A heuristic pattern is the usual case. When that happens:
   ...
   **"FIX THE CHECK" AND "NEVER PATCH THE SCRIPT" WERE THE SAME SENTENCE POINTING TWO WAYS. THIS IS THE RESOLUTION.** Where a check is a pattern in a Python file, narrowing that pattern **is** editing a script — so the rule as first written forbade the repair it mandated, and an operator meeting a wrongly-scoped blocking check had no compliant move at all. **Correcting a wrongly-scoped check is not working around it: the check is the defect.** All five conditions must hold. **If any one of them fails this does not apply, and rule 5 governs.**
   - **(a) The check has actually fired.** Never pre-emptively, never for a check you did not run.
   - **(b) You can show the wrong scope from the check's OWN source or stated contract** — a docstring, a documented exit contract, a comment describing the interaction. **That the finding is inconvenient, slow or awkward to satisfy is evidence of nothing.** If your only argument is that the translation is fine, you have not shown the check is wrong; you have asserted it.
   - **(c) You correct it in a WORKING COPY, never in the installed skill** — and you show the corrected check still catches the case it was written for. **A pattern narrowed until it fires on nothing is not a fix, it is a bypass with extra steps.**
   - **(d) You did not alter the translation to get here.** Absolute, exactly as in 5b.
   - **(e) You record it in the delivery notes** — the check, why it is wrongly scoped, what you changed, and **that the INSTALLED skill still carries the defect**, so the reader knows the next run blocks the same way.

   **You are correcting this run, not the skill.** The shipped artefact is unchanged and will fail identically next time. Saying so in the notes is what turns a local repair into a maintenance report instead of a private workaround.

   **AND CHECK 5a BEFORE 5b, NOT AFTER.** A blocking check that is wrongly scoped looks exactly like 5b's situation from the outside — every repair forbidden, no way forward — and it is not one, because a compliant repair exists: correct the check. **Read the check's source before concluding that no repair exists.** Reasoning about a check from the outside is how a scope defect gets mistaken for a deadlock.
...
| `validate_apply.py` | **MANDATORY pre-apply (post-write check inside apply) AND pre-repack** — Compare declared translations in paragraphs.json against the applied document.xml; catches the "dropped date" class of defect where character-fragmented bracket-plus-date ins clusters lose their date tokens. Use `--strict` to block on any miss. Auto-runs from `apply_translations_textmatch.py` (post-write) AND from `repack_docx.py` (pre-bundle — `--paragraphs` is REQUIRED there, and the repack refuses to bundle without it rather than skipping the check). Also supports `--report-clusters` mode that inspects `tc_cluster_hits` flags without requiring document.xml, and `--apply-zwsp` which injects ZWSPs into cluster-flagged en strings as belt-and-suspenders protection behind the apply-time auto-ZWSP. |
| `quality_check.py` | Comprehensive quality audit (target: zero issues). **Exits 2 when it reports any issue and 0 when clean**, so its verdict reaches `verify_diligence.py` at Step 11a instead of being discarded. If it will not go clean, `skill-docs/08-aux-and-quality.md` routes what happens next: five attempts, then rule 5a for a false positive or rule 5b for a real defect with no compliant repair |
