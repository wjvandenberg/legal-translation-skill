...
TWO SHAPES ARE DECLINED BY DECISION RATHER THAN BY DEFECT, and they are marked so below: a
ROMAN-NUMERAL page number (front matter) and a PREFIXED one such as `A-3` (schedules). Both
translate to themselves, so both are placeable in principle. Widening the page-number test to
admit them is a change to a shipped tree and belongs to whoever decides it rather than whoever
notices it -- so the expectations here record the CURRENT contract, and widening it must then
...
CASES = [
    ...
    ("roman-numeral page (front matter)",
     ...
     "DECLINED", "BY DECISION, NOT BY DEFECT: `post` is not digits. A roman numeral does "
                 "translate to itself, so this IS placeable in principle — widening the "
                 "page-number test to admit it is a change to a shipped tree and must be "
                 "decided, not noticed. Front matter is where this bites"),

    ("prefixed page number A-3",
     entry("A.1", "Schedule of works", "A-3"),
     "A.1Schedule of worksA-3", "A.1Schedule of works ENA-3",
     "DECLINED", "BY DECISION, NOT BY DEFECT: same as the roman numeral. Schedules and "
                 "annexes number their pages this way, and `A-3` translates to itself too"),
...
print("  prove, including two it declines BY DECISION rather than by defect.")
print("=" * 100)
sys.exit(0)
