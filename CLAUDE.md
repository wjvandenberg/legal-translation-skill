...
> touched**, and the weight that remains sits in **§2 (155), §3 (137) and §6 (147) — 439 lines no phase was
...
### HANDOFF — 2026-09-02. INSTRUMENTS DONE. NEXT IS BRANCH 7, NOT STARTED, AND ITS PROMPT IS WRITTEN

**Branch 6 is closed — PR #62 (`d3c7f19`) and its closing PR #63 (`04b7b39`).** This branch is instruments only: it promotes the eight probed table-of-contents shapes into the committed sweep and **fixes I-21 and I-22**, which were open for exactly one day. **No shipped tree file is touched**, so both pins stay at **`d3c7f19`** — verified by `git log --oneline -1 -- uk us` returning it, not assumed from the absence of a diff.

**THE SWEEP IS 26 SHAPES: 14 PLACED · 11 DECLINED · 1 SKIPPED**, and the arithmetic is now asserted because it did not add up. Wouter asked *"did you cover all types of TOCs?"* a second time, and **eighteen of eighteen could not answer it** — a sweep proves coverage of its POPULATION and nothing else, which is the same claim as 26 of 26 over D06's house style. **Measured against apply at `79a8c14`, the commit before branch 6 began: all eight new shapes declined there and none placed.** So branch 6 had made **four silent improvements** nobody had measured — a roman ENTRY number, a lower-case `a-3`, the five-atom shape **inside a table cell**, and an indented sub-entry that now keeps a leading tab the old code destroyed.

**THREE OPEN PAGE-NUMBER EDGES, NOT ONE — the count in the previous handoff was wrong by two.** `Sch-3`, a page **range `4-7`**, and a trailing full stop **`4.`** all decline **by decision**: each translates to itself, so each would place if admitted, and each spends more of the margin the digit test protects. **Three others decline by NECESSITY** and are branch 15's, not edges anyone can decide: a number and title in ONE run, an entry with no page number, and the indented sub-entry (six atoms, not five).

**AND THE INDENTED SUB-ENTRY BROKE THE VERDICT TEST, which is the more useful half.** It declines while PRESERVING its leading tab, so its atoms are `['tab', 'text']` — and `DECLINED` was tested as `atoms == ['text']`, so the row read **BROKEN**: a verdict that says *damage* where the truth is *a decline plus a repair*. **A decline means the English collapsed into ONE run, whatever structural runs survive beside it.** The summary counters held a SECOND copy of that rule and disagreed with the per-row test — **14 + 10 + 1 against 26 shapes, a sum that does not add up and that nothing asserted.** Both now read one list; the arithmetic is asserted; and the assert was proved to fire by dropping a verdict.

**I-21 FIXED AT THE CLASS, NOT THE EXTENSION.** `leakage_scan.scan()` now tests `zipfile.is_zipfile()` and reads the members, so `.docx`, `.xlsx`, `.pptx`, `.zip` **and the shipped `.skill` archive** are covered by the same three lines — a filename check would have fixed one of five. A hit reports `member:line`, an unreadable member is **reported** rather than skipped, and a clean container says `clean (N member(s) read)`, because the whole finding was that one word covered both a file that was read and a file that was not. **Proved by the red-first probe flipping:** `temp/probe_leak_docx.py` exited 1 with the needle undetected in DEFLATE, and exits 0 with it detected in **both** arms and the decoy quiet in both.

**I-22 FIXED, BOTH HALVES.** Every file is decoded **once, up front**; an undecodable one becomes a **declared SKIP** printed by name with its reason; the top line reads `FILES REQUESTED` and a `FILES READ: n of m requested` line is printed **LAST**, so a crash can no longer sit between a count and the work it claims to describe. **Scope deliberately unchanged — it stays a PROSE instrument** and points at `leakage_scan` for a container's contents. Proved on the exact invocation that crashed: previously a traceback after `FILES READ: 19`, now **exit 0, two named skips, `FILES READ: 2 of 4 requested`**.

**AND I-21's ROW OVER-CLAIMED A SECOND BLIND INSTRUMENT — CORRECTED IN PLACE, NOT DELETED.** It said `audit_branches`' `B1.clean` *"reads the same bytes and is therefore the same blind report"*. **False:** that check already unzips *(`audit_branches.py:441-445`)*, measured at 4 members and 2,049 characters with a synthetic needle visible through it. **The claim was reached BY ANALOGY from the defect next door rather than by opening the file** — this register's most-repeated lesson, applied to its own prose.

**THE SINGLE NEXT ACTION: branch 7, the container inventory** — A16 A19 N1 C19, one explicit tested inventory **SHARED** by extraction and apply, failing loudly on anything unlisted. **Its four fixtures already exist in `tests/fixtures/containers.docx` and not one is reproducible from the corpus.** The session prompt is written to `temp/NEXT-SESSION-PROMPT.md`.

**Open, none blocking.** **(1)** the three page-number edges above — decisions, not defects. **(2)** C16's residue, PARTIAL by decision, plus A20's corpus measurement and the open question whether D04's double space is a defect in apply at all. **(3)** the commit-identity decision: `verify_code` fails — **105 of 198 identity slots** are not the GitHub noreply address, **53 on a non-GitHub domain**, on a repo public since 2026-08-07; `author_allow_before` would grandfather it, but setting it ACCEPTS a published exposure, so it is a §3 decision with a trigger. **(4)** `audit_branches`' other claims are unswept for the live-inventory shape §5.16 names. **(5)** register **G12**'s 16 unclassified findings. **(6)** G9's first half, needing branch 15. **(7)** `--original` absent from Step 9's step document. **(8)** a `probe` origin class for the register. **(9)** **I-7 to I-10**, the four open A1 harness defects — and §2.3 explains why "four open" must still not be quoted as "four Step C risks". **(10)** `tools/run_tests.py` collides by name with `tests/run_tests.py`. **(11)** three `tests/` suites need the system interpreter for `lxml`. **(12)** the scan list needs tightening. **(13)** the companion-file rename is due. **(14)** `us/…/apply_translations_textmatch.py` carries the UK spelling `behaviour` at **line 1030** — the reconciliation's. **(15)** `verify_code`'s `gitignore covers`: `.env` and `secrets.json` not ignored. **(16)** its `negative tests exist`: **8 suites assert only success**. **(17)** `temp/rebuild_arithmetic.py` holds a STALE COPY of the option map — the live source is `tools/stepb_verify.py`.

**What a new session would get wrong.** It would read a sweep's pass rate as coverage; **26 of 26 is a statement about a list somebody wrote**. It would take `DECLINED` to mean every tab vanished — it means the English collapsed into one run, and a preserved leading tab is a repair, not damage. It would assert that a second instrument shares a defect **by analogy**; I-21 did exactly that and was wrong within a day. It would read the F16 zero as *already fixed*; the corpus **cannot** hold that shape. It would scope C17 from D08, which carries none of its three instances. It would hand-edit sections 9.1 or 9.2 of `STEP-B-ANALYSIS.md`, which are **generated from `tools/stepb_verify.py`**. It would compare delivered text with `para_texts`, which `.strip()`s away exactly what C17 restores. It would point a test's apply at `tests/fixtures/`. It would trust `audit_branches`' fixture claim as historical when it reads the **index**, so a new fixture must be **STAGED** first. It would read `$?` after a pipe. It would run `verify_charter_continuity.py` without `LT_HOUSE_TEMPLATES`, whose check 7 then reports **VOID, not a pass**. **Two reds are BY DESIGN** — `verify_md`'s `file length` while §1.7's exemption stands, and `stepb_audit` check 10 at 8 unverified quotations. **`verify_code` is a third red and fails THREE checks**, items (3), (15) and (16).
