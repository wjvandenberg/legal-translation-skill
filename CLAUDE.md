...
| **`FINDINGS-REGISTER.md`** | the evidence base — **228 rows**, every finding with its per-document proof. **The fastest way to understand what the project has learned** | whenever you need the proof behind any claim | every finding, clustered by root cause |
...
### HANDOFF — 2026-09-09. BRANCH 7 IS CLOSED. A19 CLOSED, SLICE 3 BUILT. NEXT IS BRANCH 8

**PR #68 merged first as `f6f1458`; slice 3 built on `feature/branch7-slice3-a19` off that. A19 CLOSED and BRANCH 7 with it — all four of its rows.** Two halves, because the row's remedy is two: a **REPORT** over every graphic surface in every part, in extraction's Step 2, naming the route or its absence; and a **TRANSLATION ROUTE** for header and footer alt text at **Step 8b.2b** — no new step number — on the scaffold `translate_headers_footers.py` already had and the bundling `repack --headers-footers-dir` already does. **One inventory, IMPORTED by both halves rather than restated**, which is option 1's rule applied to itself. **A surface with NO ROUTE is REPORTED, never refused:** §5.9's compliant-exit test, which decided slice 2's refusal the other way — a body drawing, a chart title and SmartArt text cannot be translated here and the source is the client's document.

**FIXTURE-ONLY EVIDENCE, DECLARED EXACTLY AS N1's WAS.** Re-derived: the corpus holds **14 graphics across 3 of 10 documents and not one attribute of prose** — `@descr` 0, `@title` 0, `v:shape/@alt` 0 — and 0 across both trees' 396 files, 396 opened, control FIRED. **The row never named the VML surface, which is a THIRD of the real population** (5 of 14 are `w:pict`). `tools/hf_corpus_diff.py` is new and measures that corpus-wide zero, with a **planted `@descr` proving it a real zero**.

**BOTH OWED MEASUREMENTS DISCHARGED, and one changed shape.** *(a)* The header/footer inline `sdt` was **RUN, and the reading held** — nothing stranded. The corpus cannot settle it: its one instance is doc 6, a footer, and **that paragraph's frozen `en` is null**, so apply never rebuilds it. That is §5.7's **THIRD** reason — mechanism, not damage — and **deliberately not a fifth** *(Wouter)*. *(b)* `w:dataBinding` is **3 elements across 2 documents, ALL IN FOOTERS**, sharper than the decision log; preserved intact, and the Word-in-the-loop half is **declared VOID with what it would take written down.**

**THE SINGLE NEXT ACTION: open slice 3's PR, get it approved and merged, then close branch 7 and start BRANCH 8** — reading-apart completeness, C28 C12 M1. **The queue stays in order** *(Wouter, 2026-09-09)*: 8, then 9 — which unblocks 11 — then 10, then 11. Register T6 calls C18 the highest-value single item and it is branch 11's.

**Open, none blocking.** **(1)** slice 3's PR. **(2)** whether Word repopulates a bound control from `customXml` on open — needs Word in the loop; the dev host has Word 16.0 already. **(3)** whether the upstream smart-tag removal is correct. **(4)** NEW, and not investigated: on the 3 frozen workdirs that match a corpus document, the header/footer scaffolds went partly or wholly **UNFILLED** — workdir 10 has 5 of 5 unfilled, workdir 8 has 1 of 3. If those are delivered runs, header/footer text shipped untranslated. **Not established** — 7 of 10 workdirs match nothing and hold 0–1 entries, so some are stubs or arms. **(5)** branch 6's residue: C16, A20's corpus measurement, the commit-identity decision (**read its figure from `verify_code`, never from this handoff**), G12's 16 unclassified findings, the scan list's false-positive rate, `verify_code`'s other two reds.

**SESSION COST: not measured** — Wouter reads it at the close and Claude may never estimate it.

**What a new session would get wrong.** It would match an OOXML element on its **LOCALNAME** where the name is shared — `a:t`, `w:t` and `dgm:t` all end in `t`, and that doubled a chart part's reported surfaces and mislabelled every one; the rule is now in `.claude/rules/ooxml.md`, which is **path-scoped and good for one use per session**. It would assert `"ns0:" not in output` and read a **pre-existing** `w:dataBinding` prefix mapping as a namespace rebind — a needle must be **source-relative** to fail for the right reason. It would extend `render_diff` to manufacture a visual arm: **neither A19 surface renders on any page, in this renderer or in Word**, alt text being spoken rather than painted, so the proof is in bytes. It would take a green `apply_corpus_diff` as evidence — **that tool drives APPLY**, and slice 3 changed neither apply nor repack. It would copy `uk/scripts/extract_paragraphs.py` over the us one and erase **2** lines of variant spelling; `translate_headers_footers.py` diverges in **3** (measured this session, previously unmeasured) and the step doc in **9**. It would run `tests/make_fixtures.py` with bare `python` and rewrite every fixture's bytes *(I-25)*. It would forget to **stage** a new fixture before `audit_branches` counts it — that claim has now caught **three consecutive slices** — or to refresh `sha256_now` **and** `changed_by` in `baseline-divergences.json`, where `changed_by` is the **branch name** until merge and where stamping a file this branch did not touch is worse than a stale hash. It would lift an example string out of a step document into a fixture: §5.4 requires every fixture string to be **invented for the purpose**, and `leakage_scan` caught exactly that here. **It would run `verify_charter_continuity.py` with `LT_HOUSE_TEMPLATES` at `…\Coding\templates` instead of `…\Coding\templates\evidence`, where check 7 FAILS.** **Two reds are BY DESIGN** — `verify_md`'s `file length` while §1.7's exemption stands, and `stepb_audit` check 10 at 8 unverified quotations. **`verify_code` is a third and fails three.** **Four pre-existing leakage hits stand**, two per tree, in `08-aux-and-quality.md` (pattern #46) and `translate_headers_footers.py` (pattern #19) — **proved pre-existing by scanning the `HEAD` versions**, same patterns and same shas, the second merely displaced 38 lines by this slice's insertion. Do not re-judge them as new.
