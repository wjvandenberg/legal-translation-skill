...
### HANDOFF — 2026-09-02. BRANCH 6 SLICE 2 IS BUILT AND NOT MERGED: THE TABLE OF CONTENTS IS REPAIRED
...
**(2) THE PAGE-BY-PAGE READ, which is the half no script here can discharge.** Three PNGs — old, new and source of D06 page 2 — sit in `<logs>/branch6-render/D06/` with a corrected `READ-ME.txt`. **Claude has not looked at them and may not.** Look for the dot leader and the right-aligned number returning, and specifically for **no wrapped line** on a long title: a tab placed at the paragraph end forced a wrap last time, and a misplaced tab is worse than a missing one.

**(3) A3 is PARTIAL and A6 has moved to branches 15–17.** Both maps, both appendix tables, the register and `A3-STRUCTURAL-ANALYSIS.md` now agree; option 1 fell 17 → 16 findings and option 3 rose 20 → 22, re-derived from the map rather than adjusted.

**What was learned, and it changed the build rather than the notes.** The specification's offset — `len(runs[0]["text"])` — is UNSAFE, and building the fixture Wouter asked for is what found it: **a run may carry text on BOTH sides of a tab**, so that offset places the first tab after the TITLE while the atom sequence and the tab count both still look right. The shipped rule reads the **source paragraph's own tab positions** instead; both forms were measured side by side and give identical offsets on all 26, but only one declines the bad case. **And the reason this fix was dismissed twice is a population error** — 18% over all 231 multi-fragment paragraphs, 100% over the 26 TOC entries. That wrong denominator sat in a comment in **both published trees**, telling the next reader off the thing that works. Corrected there, in section 6 of the plan, and on register row A3.
