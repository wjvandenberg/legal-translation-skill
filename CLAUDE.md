...
> **OVER-CAP EXEMPTION, RE-DECLARED 2026-09-09: 1,188 lines against a cap of 350.** *(Taken from the checker on
...
### HANDOFF — 2026-09-09. BRANCH 7 SLICE 2 BUILT, NOT MERGED. C19 CLOSED. NEXT IS SLICE 3, A19

**PR #66 merged as `9df9666`, closing slice 1.** Both pins re-derived on the new `main` and still correct: `git log --oneline -1 -- uk us` returns **`544f908`** and `git diff 544f908 -- uk us` is empty, because #66 touched no tree file. **Slice 2 is built on `feature/branch7-glossary-route` and awaits approval.**

**C19 CLOSED — the glossary gets a route IN and a route OUT, because measured, the gap was on both sides.** `--glossary` carries the part by FULL zip path · repack **REFUSES** when the ORIGINAL holds a **text-bearing** glossary and the flag is absent · `word/glossary/document.xml` joins `_PROSE_PARTS`, one of the five remnant checks the row says never looked · extraction's Step 2 aux summary now reports the part's entries, keyed by docPart NAME · **Step 8e** carries a regex-only template, **no new script** (8d's precedent; the payload is 38 characters).

**TWO OF THE ROW'S OWN STATEMENTS WERE MEASURED FALSE, and both are corrected IN the row.** *(1)* The decision's premise: repack does **not** already refuse for numbering, comments, footnotes and endnotes — those four **warn and bundle at exit 0**, four arms with both positive controls firing. Copying the named shape would have built the control that already failed. Wouter was shown the measurement and confirmed the block; the shape copied is the `--paragraphs` gate's. *(2)* The reference element: `docPartObj` and `docPartGallery` are **0** on the carrier, against `docPart` 10, `placeholder` 20, `w:sdt` 10, and **10 of 10 docPart names verbatim in `document.xml`**. The mechanism is `w:placeholder/w:docPart` — the prompt text Word shows while a control is EMPTY. **Second fixture on this branch built from a row's words and carrying a shape the corpus does not have.**

**ACCEPTANCE: `tests/test_glossary_route.py`, 38 checks, 0 void, both variants**, red first in exactly the arms the fix changes, with the conforming pair (empty glossary, no glossary) green throughout. **The corpus arm reported 13 byte-identical, 0 unexplained, and that proves NOTHING about this slice** — `apply_corpus_diff` drives apply, this changes repack. Declared, not presented as coverage.

**THE DEFECT HAS NO PAGE IN THIS RENDERER, measured with a control firing.** LibreOffice renders no glossary placeholder text: translated and untranslated give **byte-identical page text**. A three-arm visual diff would have been false coverage. What the render proves is that the package LOADS — I-17's family, a glossary part needing both its relationship and its content-type override. **That invisibility is close to why the defect survived.**

**THE SINGLE NEXT ACTION: merge PR #67, then slice 3 — A19.** Report every graphic surface and **translate the alt text** *(§2.6, 2026-09-08)*. The corpus has 9 `w:drawing` and 5 `w:pict`, **all in headers and footers, and not one `@descr`** — so the translation is proved on a fixture alone and that must be said. `translate_headers_footers.py` has an `extract_to_scaffold` → `apply_from_scaffold` shape and already handles `txbxContent` through `_iter_own_runs`; that is where alt text fits. **Re-derive the zeros before building.** Wouter's decision of 2026-09-09: **the two owed measurements ride with slice 3** — the header/footer `sdt` surface (5 across 4 documents; the "strands nothing" claim is a READING, and this project's record on readings is poor) and the `w:dataBinding` question (2 documents; needs Word in the loop, so slice 3 writes what that would take and builds nothing).

**Open, none blocking.** **(1)** slice 3, above. **(2)** `quality_check.py` and `verify_diligence.py` still do not scan the glossary — **deliberately**, by decision: register T6 gives the general aux-part content diff and the whole-package language sweep to **C18, branch 11**. Repack's own post-repack scan covers the delivered part meanwhile. **(3)** **A22 is a DEADLOCK, not a duplication** — `validate_apply --strict` refuses the repack — and gets no branch, because fixing it reopens clause 3's keyword narrowing; zero corpus instances. **(4)** whether the upstream removal of a smart tag is CORRECT. **(5)** everything branch 6 left: C16's residue, A20's corpus measurement, the commit-identity decision (**104 identity slots examined, 53 outside the allow-list across 2 distinct addresses** — re-derived from `verify_code` on 2026-09-09; the previous handoff said 105 and it was carried forward, not measured. A §3 decision with a trigger, and one that grows with every commit, so **read it from the checker, never from the last handoff**), G12's 16 unclassified findings, the scan list's false-positive rate, `verify_code`'s other two reds.

**What a new session would get wrong.** It would read `test_baseline_unmodified.py`'s PASS as covering its own edits — **it reads the git INDEX, so an unstaged tree edit is invisible to it**; eight edited files went green here until they were staged, and the suite now prints which tree it read and names unstaged files. It would build a fixture from a register row's TITLE, twice over. It would refute a behavioural claim from structure. It would take a green `apply_corpus_diff` as evidence about a repack change. It would extend `render_diff` to manufacture a visual arm for a defect no renderer can show. It would key an aux-part inventory on a BASENAME — `word/glossary/document.xml` and `word/document.xml` collapse, and that once produced a written "C19 did not recur". It would copy `uk/scripts/extract_paragraphs.py` over the us one and erase two lines of variant spelling — that file **diverges**, `repack_docx.py` does not. It would run `tests/make_fixtures.py` with bare `python` and rewrite all seventeen fixtures' bytes *(I-25)*. It would forget to **stage** a new fixture before `audit_branches` counts it, or to refresh `sha256_now` **and** `changed_by` in `baseline-divergences.json` — where `changed_by` is the **branch name** until merge, by that file's own rule. **It would run `verify_charter_continuity.py` with `LT_HOUSE_TEMPLATES` set to `…\Coding\templates` instead of `…\Coding\templates\evidence`, where check 7 FAILS.** **Two reds are BY DESIGN** — `verify_md`'s `file length` while §1.7's exemption stands, and `stepb_audit` check 10 at 8 unverified quotations. **`verify_code` is a third and fails three checks.**
