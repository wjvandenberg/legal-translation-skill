...
### HANDOFF — 2026-09-01. BRANCH 6 IS MERGED: "STOP DELETING", AND THE TOC IS MEASURED BUT NOT BUILT

**Merged as `4a1c452` (PR #57), five commits, 16 files.** Option 1's first slice closes **A1** (footnote anchors), **A2** (comment anchors), **A8** (hyperlink wrappers — 34 of 34 verified to contain text, so they genuinely navigate), **A9** (D06's 45 duplicated cross-references), **F27** (authored boundary separators) and **A3/D01** (the doubled-tab finding: 24 → 18, exactly source). C16, C17 and F16 remain a declared follow-on slice.

**THE ACCEPTANCE CONDITION WAS INVERTED AND IT HELD:** `tools/apply_corpus_diff.py` runs apply twice over all 13 frozen intermediates and judges each movement against **the SOURCE document's own count**. 0 unexplained. **The pins in that tool and in `render_diff.py` now point at `4a1c452`** — moved after the merge, because a pin left at the previous baseline makes the next branch's diff report branch 6's own work as new.

**WOUTER'S RENDER REVIEW FOUND THREE OF THIS BRANCH'S FOUR REAL DEFECTS, AND NONE WAS FOUND BY READING.** A misplaced tab is worse than a missing one: emitting a between-text tab at the paragraph end glued D06's page numbers exactly as before AND forced a line wrap. The rule is now *keep a tab only where its true position survives the collapse*. He also caught `render_diff.py` handing him a **stale page as evidence** — the directory was not cleared, so page 2 from the regression run sat beside 87 pages from the run that fixed it. Cleared now, plus `--pages` and a `MANIFEST.txt`, because absence was ambiguous.

**THE TABLE OF CONTENTS CAN BE REPAIRED, AND I TWICE SAID IT COULD NOT.** The affix measurement that killed the idea was taken over **all 231 multi-fragment paragraphs** (18%); over the **26 TOC entries**, the population the fix is for, it is **100%** — §5.1's own warning that a ported check is a check against a new population. Measured: fragment 0 is an exact prefix of `en` on 26/26, the trailing digit run is identical in `text` and `en` on 26/26, both non-overlapping on 26/26, and **0/3 on D01's TOC-shaped lookalikes**, so the rule already has a negative control. `en_runs` is empty on all of them and cannot supply the boundary — that route was checked and is closed.

**THE SINGLE NEXT ACTION: build that TOC placement, as a fresh session's FIRST act.** Not built in the session that measured it because context was ~90% and a heavy step in the back half leaves a half-applied change in a tree that ships. **The full specification is section 7 of `temp/PLAN-branch6-stop-deleting.md`** — the two offsets, the four steps, and why the earlier number was wrong. `tests/fixtures/toc.docx` already reproduces the shape and renders; `render_diff.py --fixture toc` is the instrument that will SHOW the dot leaders return. **Then branch 7, the container inventory** (A16 A19 N1 C19), whose four fixtures already exist in `containers.docx`.

**§5.3's GATE NOW HAS A TOOL, AND ITS SHAPE IS FORCED BY §6.5.** A rendered page of a corpus document is client text as an image, and the transcript is the one leak class no control here reaches — so **Claude may never look at a render of a real document**. Real corpus mechanically, synthetic fixtures visually, and judging real pages by eye stays a human's job. D06 36→36 pages and D05 20→20, both matching source.

**AND THE COUNT IS NOT THE CRITERION.** D02 lost 45 tabs and **not one pixel moved** on any of its 11 pages: a stranded tab advances into empty space, and a comment anchor is not printed. §2.5 item 7, measured. So `tab_chars` is a PARTIAL row pointing at `render_diff`, not a number to satisfy.

**Open, none blocking.** **(1)** the TOC placement, specified above. **(2)** C16 C17 F16. **(3)** A3/A6 still sit on branch 6's row in `STEP-B-ANALYSIS.md`'s section 9.3 while the code defers them — move them or let the TOC work close them. **(4)** register **G12**'s 16 unclassified findings. **(5)** G9's first half, needing branch 15. **(6)** `--original` absent from Step 9's step document. **(7)** a `probe` origin class for the register. **(8)** **I-7 to I-10**, the four open A1 harness defects. **(9)** `tools/run_tests.py` collides by name with `tests/run_tests.py`. **(10)** three `tests/` suites need the system interpreter for `lxml`. **(11)** the scan list needs tightening. **(12)** the companion-file rename is due. **(13)** `apply_corpus_diff.py` and `render_diff.py` duplicate their source-matching helpers, declared in the latter's docstring.

**What a new session would get wrong.** It would trust a prompt's line-ending map: measured over all 493 tracked files, `uk/` and `us/` are pure LF, the root, `.claude/`, `tests/` and `tools/` are each MIXED **per file**, and only the root `.md` documents are reliably CRLF — so measure the file, not the directory. The Edit tool **does** match a multi-line string in a CRLF file; that warning is false. It would run `verify_charter_continuity.py` without `LT_HOUSE_TEMPLATES` set to `…\Coding\templates`, whose check 7 then reports **VOID, not a pass**. It would read `verify_md`'s `file length` FAIL as a regression — by design while §1.7's exemption stands. And it would assume `apply_corpus_diff.py` works in a fresh clone: it reads the corpus location from gitignored `.claude/evidence-dirs.local`, and without that file the real arm is **unavailable rather than clean**.
