...
### HANDOFF — 2026-09-01. BRANCH 6's FIRST SLICE IS BUILT: "STOP DELETING"

**Produced: the first delivered byte this project has ever deliberately moved.** Option 1's three clauses in `apply_translations_textmatch.py`, both trees, closing **A1 A2 A3 A6 A8 A9 F27**. C16, C17 and F16 are a declared follow-on slice *(Wouter, 2026-09-01: they are boundary-whitespace and offset defects in three other functions, not deletions)*. Not merged — §3.1's row is the status.
...
**F27's TWO EVIDENCE DOCUMENTS CANNOT DEMONSTRATE IT, MEASURED.** Across all 1,891 frozen notes entries there are **ZERO** boundary tabs, so D01 and D10 come out byte-identical and only the synthetic fixture can test that half. The five boundary NEWLINES that do exist are on D06 and D09, and they are exactly the movement those two documents show.
...
**What a new session would get wrong.** It would trust a prompt's line-ending map: `tests/` and `tools/` are **NOT** CRLF — both conventions exist **per file**, LF in the majority, and only the root `.md` documents are uniformly CRLF, so measure the file before editing it. It would run `verify_charter_continuity.py` without `LT_HOUSE_TEMPLATES` set to `…\Coding\templates`, whose check 7 then reports **VOID, which is not a pass**. It would read `verify_md`'s `file length` FAIL as a regression — it fails **by design** while §1.7's exemption stands. And it would assume `tools/apply_corpus_diff.py` works in a fresh clone: it reads the corpus location from gitignored `.claude/evidence-dirs.local`, and without that file the real arm is **unavailable rather than clean**.
