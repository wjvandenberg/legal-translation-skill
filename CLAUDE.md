...
| **`FINDINGS-REGISTER.md`** | the evidence base — **227 rows**, every finding with its per-document proof. **The fastest way to understand what the project has learned** | whenever you need the proof behind any claim | every finding, clustered by root cause |
...
> **OVER-CAP EXEMPTION, RE-DECLARED 2026-09-09: 1,186 lines against a cap of 350.** *(Taken from the checker on
...
### HANDOFF — 2026-09-09. BRANCH 7 SLICE 2 MERGED. C19 CLOSED. NEXT IS SLICE 3, A19, THE LAST

**Slice 2 merged as `ae48f6d`, PR #67. C19 CLOSED.** The glossary part now has a route IN and a route OUT, because measured, the gap was on both sides and the row frames it as repack's alone: `--glossary` carries the part by FULL zip path · repack **REFUSES** when the ORIGINAL holds a **text-bearing** glossary and the flag is absent · the part joins `_PROSE_PARTS`, one of the five remnant checks the row says never looked · extraction's Step 2 aux summary reports its entries keyed by docPart NAME · **Step 8e**, a regex-only template on 8d's precedent, **no new script**.

**BOTH PINS MOVED TO `ae48f6d` AS THE FIRST ACT AFTER THE MERGE** — derived from `git log --oneline -1 -- uk us` with `git diff ae48f6d -- uk us` empty, never read off the merge message. **Sixth move in five days.** Slice 2 is the case where forgetting would have been hardest to see: it changed repack and extraction, **not apply**, so `apply_corpus_diff`'s arms were byte-identical by design and it reported 13 of 13 unchanged — a stale pin would have gone on producing that same correct-looking all-quiet run over a baseline missing eight tree files of change. **The reading that catches a stale pin is the one that MOVES, and this slice produced none.** No `FIX_LANDED` row is owed: slice 2 added no directional arm to that tool, and both existing rows re-checked as ancestors of the new pin.

**TWO OF C19'S OWN STATEMENTS WERE MEASURED FALSE and both are corrected IN the row.** *(1)* The decision's premise: repack does **not** already refuse for numbering, comments, footnotes and endnotes — those four **warn and bundle at exit 0**, four arms with both positive controls firing. Copying the named shape would have built the control that already failed here. Wouter confirmed the block on the measurement; the shape copied is the `--paragraphs` gate's. *(2)* `docPartObj` and `docPartGallery` are **0** on the carrier, against `docPart` 10, `placeholder` 20, `w:sdt` 10, **10 of 10 docPart names verbatim in `document.xml`** — the mechanism is `w:placeholder/w:docPart`, the prompt text Word shows while a control is EMPTY. **Second fixture on this branch built from a row's words and carrying a shape the corpus does not have.**

**THE SINGLE NEXT ACTION: slice 3 — A19, and it closes branch 7.** Report every graphic surface and **translate the alt text** *(§2.6, 2026-09-08)*. **Wouter, 2026-09-09: fixture-only evidence, DECLARED exactly as N1 was** — the corpus has 9 `w:drawing` and 5 `w:pict`, all in headers and footers, and **not one `@descr`**, so there is no real-document instance of translatable graphic metadata in any form and that must be said rather than left to look like corpus evidence. **Re-derive the zeros before building** — docPr, SmartArt, `w:drawing`, `a:graphic`, `c:title`, `wp:inline`, diagrams were all 0 across both trees on 2026-09-08, with a control needle firing. `translate_headers_footers.py` has an `extract_to_scaffold` → `apply_from_scaffold` shape and already handles `txbxContent` through `_iter_own_runs`; that is where alt text fits, the corpus drawings all being in header and footer parts. **The two owed measurements ride with slice 3** *(Wouter, 2026-09-09)*: the header/footer `sdt` surface (5 across 4 documents — the "strands nothing" claim is a **reading**, and this project's record on readings is poor) and the **`w:dataBinding`** question (2 documents; needs Word in the loop, which no instrument has, so slice 3 writes what that would take and **builds nothing**).

**Open, none blocking.** **(1)** slice 3, above. **(2)** the queue stays **in order** *(Wouter, 2026-09-09)*: after branch 7 come 8, then 9 — which unblocks 11 — then 10, then 11. **Register T6 calls C18 the highest-value single item in the register**, and it is branch 11's; branch 9's change journal is what makes 11 buildable, one of §3.2's three absolute sequencing facts. **(3)** `quality_check.py` and `verify_diligence.py` still do not scan the glossary, **deliberately, by decision** — T6 gives the general aux-part content diff and the whole-package language sweep to C18. Repack's own post-repack scan covers the delivered part meanwhile. **(4)** **A22 is a DEADLOCK, not a duplication** and gets no branch, because fixing it reopens clause 3's keyword narrowing; zero corpus instances. **(5)** whether the upstream removal of a smart tag is CORRECT. **(6)** everything branch 6 left: C16's residue, A20's corpus measurement, the commit-identity decision (**read the figure from `verify_code`, never from this handoff** — it grows with every commit), G12's 16 unclassified findings, the scan list's false-positive rate, `verify_code`'s other two reds.

**SESSION COST: 46%, read by Wouter at the close and not estimated** — one whole sub-step (explore, plan, build, verify, test, merge, close), inside the 26–54% band the fourteen recorded readings now span. **Recorded, never obeyed:** the session ended because its planned work ran out.

**What a new session would get wrong.** It would read `test_baseline_unmodified.py`'s PASS as covering its own edits — **it reads the git INDEX**, and eight edited tree files went green here until they were staged; the suite now prints which tree it read and names unstaged files. It would build a fixture from a register row's TITLE, which has now misled twice on one branch. It would refute a behavioural claim from structure. It would take a green `apply_corpus_diff` as evidence about a repack change — **that tool drives APPLY**. It would extend `render_diff` to manufacture a visual arm for a defect no renderer can show: **LibreOffice renders no glossary placeholder text at all**, measured with a control firing, so translated and untranslated give byte-identical page text. It would key an aux-part inventory on a **BASENAME** — `word/glossary/document.xml` and `word/document.xml` collapse, and that once produced a written "C19 did not recur". It would copy `uk/scripts/extract_paragraphs.py` over the us one and erase two lines of variant spelling — **that file diverges, `repack_docx.py` does not.** It would run `tests/make_fixtures.py` with bare `python` and rewrite all seventeen fixtures' bytes *(I-25)*. It would forget to **stage** a new fixture before `audit_branches` counts it — that claim has now caught two consecutive slices — or to refresh `sha256_now` **and** `changed_by` in `baseline-divergences.json`, where `changed_by` is the **branch name** until merge, by that file's own rule, and where stamping a file this branch did not touch is worse than a stale hash. **It would run `verify_charter_continuity.py` with `LT_HOUSE_TEMPLATES` at `…\Coding\templates` instead of `…\Coding\templates\evidence`, where check 7 FAILS.** **Two reds are BY DESIGN** — `verify_md`'s `file length` while §1.7's exemption stands, and `stepb_audit` check 10 at 8 unverified quotations. **`verify_code` is a third and fails three checks.** **One pre-existing leakage hit stands in `08-aux-and-quality.md`** on pattern #46 in both trees — same pattern and sha as on `main` before this branch, judged and displaced, not introduced.
