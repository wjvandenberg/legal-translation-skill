...
**BOTH OF THIS SLICE'S CLOSING DECISIONS ARE TAKEN — `DECISIONS-LOG.md`, 2026-09-02, owns the reasoning and it is not summarised here.** *(1)* The C16 fix **keeps both halves** — *at most one character, and only where the boundary has no whitespace from either side* — the second half having been forced by a mechanism the plan did not predict. It shipped before it was confirmed, with the overrun flagged in the PR, the commit and the closing exchange; that entry records why that is defensible here and would not be for anything irreversible. *(2)* **C16 stays PARTIAL, its residue gets no branch, and the queue is NOT reordered.** **Neither is open — do not re-ask either.**
...
**Open, none blocking.** **(1)** the C16 residue — **PARTIAL by decision, not by oversight** — plus A20's corpus measurement, and the open question the decision deliberately did NOT settle: whether D04's delivered double space is a defect in apply at all, or inherited from a source that carries twelve. **(2)** the commit-identity decision — `verify_code` fails: **105 of 198 identity slots** are not the GitHub noreply address, **53 on a non-GitHub domain**, on a repo public since 2026-08-07; `author_allow_before` would grandfather it, but setting it ACCEPTS a published exposure, so it is a §3 decision with a trigger. **(3)** `audit_branches`' other claims are unswept for the live-inventory shape §5.16 names. **(4)** whether the page-number prefix edge moves past two letters. **(5)** register **G12**'s 16 unclassified findings. **(6)** G9's first half, needing branch 15. **(7)** `--original` absent from Step 9's step document. **(8)** a `probe` origin class for the register. **(9)** **I-7 to I-10**, the four open A1 harness defects. **(10)** `tools/run_tests.py` collides by name with `tests/run_tests.py`. **(11)** three `tests/` suites need the system interpreter for `lxml`. **(12)** the scan list needs tightening. **(13)** the companion-file rename is due. **(14)** `us/…/apply_translations_textmatch.py` carries the UK spelling `behaviour` at **line 1030** (it moved from 937 — the reconciliation's to fix, not a new defect). **(15)** `verify_code`'s `gitignore covers`: `.env` and `secrets.json` are not ignored. **(16)** its `negative tests exist`: **8 suites assert only success** — and this is the check that says whether a test can fail at all, so it is not cosmetic. **(17)** `temp/rebuild_arithmetic.py` holds a STALE COPY of the option map (option 3 at 20, option 5 at 45, against a live 23 and 46) — it is gitignored scratch, but anything quoting it gets a wrong answer; the live source is `tools/stepb_verify.py`.
