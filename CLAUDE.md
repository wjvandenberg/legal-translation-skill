...
| **`FINDINGS-REGISTER.md`** | the evidence base — **222 rows**, every finding with its per-document proof. **The fastest way to understand what the project has learned** | whenever you need the proof behind any claim | every finding, clustered by root cause |
...
> **OVER-CAP EXEMPTION, RE-DECLARED 2026-09-02: 1,143 lines against a cap of 350.** *(Taken from the checker on
...
> touched**, and the weight that remains sits in **§2 (156), §3 (137) and §6 (147) — 440 lines no phase was
...
>
> **SIX ARE OPEN, AND THEY ARE NO LONGER ONE SET — THE SHORTHAND THIS BLOCK USED TO OFFER IS WITHDRAWN**
...
> Step C risks' are the same set, and a session can quote either without qualifying it."* **Both of those
> sentences are now false.** Closing branch 6's fourth slice added **I-21 and I-22 — the FIRST open rows
> the second population has ever had**: two confidentiality controls that report on a file class they
> cannot read *(a byte scanner is blind inside a compressed `.docx`)* and crash on a binary after
> announcing how many files they will read. **Neither threatens Step C**, so the identity is gone and a
> session must now say WHICH six. **The shorthand is not repaired, deliberately: an equality between two
> sets that happens to hold is a claim waiting to go stale, and it went stale exactly once and cost this
> paragraph.** Quote the count from `audit_register.py`, which prints `instrument=22(16f/6o)` on every
> run, and name the population you mean.
...
### HANDOFF — 2026-09-02. BRANCH 6 IS CLOSED, ALL FOUR SLICES MERGED. NEXT IS BRANCH 7, NOT STARTED

**Merged today: PR #62 (`d3c7f19`), slice 4 — C17 and F16 CLOSED, C16 PARTIAL.** Branch 6's four slices are `4a1c452` · `049484e` · `2a71e71` · `d3c7f19`. **This closing branch does two things and no more:** moves both comparison pins to **`d3c7f19`**, and files the two control defects the merge check found. `DECISIONS-LOG.md`, 2026-09-02 owns the slice's two closing decisions and is not summarised here.

**BOTH PINS NOW READ `d3c7f19`** — verified by `git log --oneline -1 -- uk us` returning it and `git diff d3c7f19 -- uk us` coming back empty, never by reading a merge message. **The pin has moved FOUR times in three days, so moving it is the FIRST act after a merge, never a closing tidy-up.** **And the comment above a pin goes stale as readily as the pin:** `render_diff`'s block read *"Moved to `049484e`"* while its own pin one line below said `2a71e71` — the two disagreed with each other inside five lines. Nothing checks a comment. Both blocks are rewritten, not appended to.

**THE MERGE CHECK FOUND TWO CONTROL DEFECTS BY ASKING OF A FILE CLASS WHAT §5.4 ASKS OF A DIRECTORY: does a clean report come from a control that could actually reach the file?** Both are now register rows, both **open**, and neither is a leak.

- **I-21 — `leakage_scan` CANNOT READ INSIDE A COMPRESSED `.docx` AND REPORTS IT CLEAN.** Measured with a live pattern planted and never printed: in a real DEFLATE document the needle was **NOT detected**, in a byte-identical **STORED** ZIP it **fired**. So the limit is **compression, not the extension** — invisible to anyone reasoning from a filename. **`audit_branches`' `B1.clean` reads the same bytes and is the same blind report, in a claim checked on every run.** What bounds the risk is the **location rule, not the scan**: `precommit_gate` asserts 0 Word documents outside `tests/fixtures/`, and every fixture is emitted by a committed generator. **A byte scanner is blind to every CONTAINER — and the `.skill` archive is this project's own deliverable.**
- **I-22 — `descriptor_shape_sweep` CRASHES on a binary AFTER printing `FILES READ: 19`**, so the files behind the crash are silently unswept and the output reads as a partial success. Its default scope (15 prose documents) is right and is not the defect; being handed a file list is. **A control that reports what it was ASKED to do rather than what it DID is now the fourth instance of that shape here** — I-19's `$?`, I-20's exit 0 on a killed build, C29's crash announced as an intentional block.

**A ~10-LINE FIX EXISTS FOR EACH AND WAS DELIBERATELY NOT TAKEN ON THIS BRANCH** *(read a ZIP's members; declare a SKIP instead of crashing, and print the count after reading)*. **`temp/probe_leak_docx.py` is already the red-first test** and must go on failing before the fix and passing after. Scope was Wouter's call at the close: file them, do not widen a closing PR.

**HOW THE GAP WAS CLOSED FOR THE MERGE RATHER THAN EXCUSED:** both new fixtures were **decompressed** and all 8 parts put through both controls *(93 patterns, CLEAN)*, and both **rebuild BYTE-IDENTICALLY** from `tests/make_fixtures.py`. That is stronger than any scan, because a scan can only fail to find what it knows to look for.

**THE SINGLE NEXT ACTION: branch 7, the container inventory** — A16 A19 N1 C19, one explicit tested inventory SHARED by extraction and apply, failing loudly on anything unlisted. **Its four fixtures already exist in `tests/fixtures/containers.docx` and not one is reproducible from the corpus.**

**Open, none blocking.** **(1)** I-21 and I-22 above. **(2)** C16's residue — **PARTIAL by decision, not oversight** — plus A20's corpus measurement, and the question the decision deliberately left open: whether D04's delivered double space is a defect in apply at all, or inherited from a source carrying twelve. **(3)** the commit-identity decision: `verify_code` fails — **105 of 198 identity slots** are not the GitHub noreply address, **53 on a non-GitHub domain**, on a repo public since 2026-08-07; `author_allow_before` would grandfather it, but setting it ACCEPTS a published exposure, so it is a §3 decision with a trigger. **(4)** `audit_branches`' other claims are unswept for the live-inventory shape §5.16 names. **(5)** **THREE open page-number edges, not one** *(measured 2026-09-02 by `temp/probe_toc_gaps.py`, which asked what the 18-shape sweep does NOT contain)*: a three-letter prefix `Sch-3`, a page **RANGE** `4-7`, and a page number with a **trailing full stop** `4.`. All three decline **by decision, not by defect** — each translates to itself, so each is placeable if admitted, and each spends a little more of the false-positive margin the digit test exists to protect. **And the same probe measured what the sweep never said out loud: a number and title in ONE run, an entry with no page number, and an INDENTED sub-entry (six atoms, not five) all decline by NECESSITY** — that is branch 15's per-run English, not an edge. **Branch 6 improved 4 of those 8 untested shapes and damaged none** *(a roman ENTRY number, a lower-case `a-3`, and the five-atom shape inside a TABLE CELL all went DECLINED → PLACED; the indented sub-entry now KEEPS its leading tab where that tab used to be destroyed)*. **The 8 shapes are not a committed suite** — adding them is its own small piece of work. **(6)** register **G12**'s 16 unclassified findings. **(7)** G9's first half, needing branch 15. **(8)** `--original` absent from Step 9's step document. **(9)** a `probe` origin class for the register. **(10)** **I-7 to I-10**, the four open A1 harness defects — and note that "four open" is no longer the whole open set: it is **six**, and §2.3 explains why that shorthand is withdrawn. **(11)** `tools/run_tests.py` collides by name with `tests/run_tests.py`. **(12)** three `tests/` suites need the system interpreter for `lxml`. **(13)** the scan list needs tightening. **(14)** the companion-file rename is due. **(15)** `us/…/apply_translations_textmatch.py` carries the UK spelling `behaviour` at **line 1030** — the reconciliation's. **(16)** `verify_code`'s `gitignore covers`: `.env` and `secrets.json` not ignored. **(17)** its `negative tests exist`: **8 suites assert only success**. **(18)** `temp/rebuild_arithmetic.py` holds a STALE COPY of the option map — the live source is `tools/stepb_verify.py`.

**What a new session would get wrong.** It would treat a CLEAN confidentiality report over a `.docx` as coverage — it is not, and I-21 says why. It would read the F16 zero as *already fixed*; the corpus **cannot** hold that shape. It would scope C17 from D08, which carries none of its three instances. It would hand-edit sections 9.1 or 9.2 of `STEP-B-ANALYSIS.md`, which are **generated from `tools/stepb_verify.py`** — that happened, and the checker caught it. It would compare delivered text with `para_texts`, which `.strip()`s away exactly what C17 restores. It would put an offset guard where it sees a **stripped** `en_text` and refuse correct input — that happened, on D07. It would point a test's apply at `tests/fixtures/`. It would trust `audit_branches`' fixture claim as historical when it reads the **index**, so a new fixture must be **STAGED** first. It would read `$?` after a pipe. It would run `verify_charter_continuity.py` without `LT_HOUSE_TEMPLATES`, whose check 7 then reports **VOID, not a pass**. **Two reds are BY DESIGN** — `verify_md`'s `file length` while §1.7's exemption stands, and `stepb_audit` check 10 at 8 unverified quotations *(it needs `LEGAL_TRANSLATION_A4`)*. **`verify_code` is a third red and fails THREE checks**, items (3), (16) and (17).
