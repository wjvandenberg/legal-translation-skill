...
> **OVER-CAP EXEMPTION, RE-DECLARED 2026-09-02: 1,119 lines against a cap of 350.** *(Taken from the checker on
...
| **2** | **Building** (goals ii and iv) — the rest of `STEP-B-ANALYSIS.md`: branches 3 to 19, plus the three deferred items | **IN PROGRESS since 2026-08-07. MERGED: branches 3, 4, 5 and branch 14's `quality_check` slice.** Branch 5's fourth gate — the behavioural probe of rule 5b — was **discharged 2026-08-20** and branch 5 merged **2026-08-21**; `DECISIONS-LOG.md`, 2026-08-20 owns that reasoning, including that the plan was wrong to treat 5b as what makes branch 5 safe, and it must not be summarised back to here. **BRANCH 6 SLICE 1 MERGED 2026-09-01** as `4a1c452` — option 1's three clauses, closing **A1 A2 A8 A9 F27**. **SLICE 2, the table-of-contents tab placement, IS BUILT AND NOT MERGED** *(2026-09-02)*: it closes **A3's TOC and doubled-tab instances**, so **A3 is PARTIAL and A6 has moved to branches 15–17** — a glued bullet has no boundary that translates to itself. **C16, C17 and F16 remain a declared follow-on slice** *(Wouter, 2026-09-01: boundary-whitespace and offset defects in three other functions, not deletions)*. **Both slices MOVE a delivered byte, so the acceptance condition is inverted:** every movement explained finding by finding against the SOURCE count, and nothing moving that no register row predicts. The remaining branches plus the three deferred items are fully planned and fully decided | slice 2's merge: **Wouter's approval, and his page-by-page read of the rendered pages. Nothing technical remains** |
...
### HANDOFF — 2026-09-02. BRANCH 6's THREE MERGED SLICES ARE CLOSED. NEXT IS C16 · C17 · F16, PLANNED AND NOT STARTED

**Merged this session: PR #59 (`049484e`) and PR #60 (`2a71e71`).** Both comparison pins now read **`2a71e71`** — moved on this closing branch, verified by `git log -- uk us` and by `git diff 2a71e71 -- uk us` coming back empty. **The pin has moved three times in two days, so moving it is the FIRST act after a merge, never a closing tidy-up.**

**Slice 3, the page-number widening.** A contents entry's page number may now be a **well-formed roman numeral in uniform case** or **one-or-two letters + separator + digits** (`A-3`), beside arabic digits. **Its acceptance condition was INVERTED and that is the proof: 13 corpus documents compared, 0 moved, 13 byte-quiet**, with the harness's self-comparison note absent so it was a real before-and-after. The sweep runs **18 shapes, 11 placed, 6 declined**. The obvious roman pattern `[ivxlcdm]+` matches the English word **`civil`**, so the shipped rule is a well-formedness test plus uniform case, with four negative shapes holding that line and `mix` declared as a residue.

**SEEN, NOT ONLY COUNTED — AND THE FIRST ATTEMPT AT THE EVIDENCE FAILED ITS READER.** Shown the render, Wouter read the two deliberately-flat lines as damage, fairly, since nothing on the page said they were refusals. Answered by **rendering a third arm from the pre-widening script: 1 placed / 6 declined, only the first line with a leader** — so the branch fixed four lines and damaged none. `tests/fixtures/toc-widened.docx` now **labels itself on the page**, and the same labels are in the READ-ME text `render_diff` writes.

### THE NEXT SLICE IS EXPLORED AND PLANNED — `temp/PLAN-branch6-c16-c17-f16.md`. NO CODE UNTIL WOUTER SAYS GO.

**TWO OF THE THREE ROWS DO NOT MEAN WHAT THEIR TITLES SAY, measured over all 13 frozen intermediates and 1,891 notes entries** (`temp/probe_c16_c17_f16.py`):

- **C17 — 3 real instances, and ZERO on D08, the document the row names.** D02 idx 31 and 170, D07 idx 59. The row's D08 sentence is conditional — *"mirroring them **would have** rendered…"* — and that operator did not mirror. **Three others did**, and every one is silently cleared at `apply_translations_textmatch.py:458` → `:484`. **Scope from the population, never from the discovery.**
- **F16 — 0 across all thirteen, and the zero is PROVED real rather than a check that read nothing.** `temp/probe_enruns_keys.py`: 729 entries carry `en_runs`, the keys are exactly `start`/`end`/`bold`/`italic`, the last run's `end` lands on `len(en)` on **729 of 729** and on `len(text)` on 0, a planted needle **fired**, a conforming input stayed quiet. **The corpus CANNOT carry F16: the frozen intermediates are the POST-COMPLIANCE artefact (§5.8 rule 2) and `validate_en_runs.py` is a PRE-APPLY GATE**, so a run with out-of-range offsets could never have produced one. **It needs a synthetic fixture** — the TOC remedy, a different reason.
- **C16 — not measured, DECLARED, not zero.** It needs delivered-versus-declared, so it needs apply; the frozen-intermediate→source matching lives inside a tool whose module body *is* the tool. That arm belongs in the branch's own red-first instrument.

**Wouter's scope decision, 2026-09-02: all three in one branch, and F16 = the Step 4c line PLUS a loud guard in apply's `en_runs` loop** that refuses an out-of-range offset instead of clamping. That guard is inside apply and is **not** C13's range assertion, which stays branch 11's — confirm by reading C13 before building.

**THE SINGLE NEXT ACTION: get GO on that plan, then build it** — branch off the merged `main`, pins first. Then **branch 7**, the container inventory (A16 A19 N1 C19), whose four fixtures already exist in `containers.docx`.

**AND ONE OPEN ITEM IS NOT LIKE THE OTHERS, FOUND 2026-09-02 BY RUNNING `verify_code` AND TRACKED NOWHERE BEFORE THIS.** Its `commit author identity` check FAILS: **105 of 198 identity slots across the whole history are not the GitHub noreply address, and 53 carry a NON-GITHUB DOMAIN**, on a repository public since 2026-08-07. *(Domains classified in aggregate; no address printed — the count is the finding. This session's own commits are clean on both author and committer, measured.)* **An author email is metadata on every commit, so every CLEAN confidentiality report this project has produced was correct and blind to it.** The house rule is *rewrite BEFORE a repo goes public, never after*, and that window has closed. **`author_allow_before` exists to grandfather it and keep the count visible — but setting it ACCEPTS a published exposure, which is a §3 decision with a trigger, not a config change made in passing. It is deliberately left here rather than filed, because where it goes depends on what Wouter decides.**

**Open, none blocking.** **(1)** C16 C17 F16, then branch 7. **(2)** the commit-identity decision above. **(3)** **`audit_branches`' OTHER claims are not swept** for the shape §5.16 now names — any claim asserting a LIVE INVENTORY rather than a historical delivery has it. **(4)** whether the prefix edge moves past two letters (`Sch-3` is refused, by decision). **(5)** register **G12**'s 16 unclassified findings. **(6)** G9's first half, needing branch 15. **(7)** `--original` absent from Step 9's step document. **(8)** a `probe` origin class for the register. **(9)** **I-7 to I-10**, the four open A1 harness defects. **(10)** `tools/run_tests.py` collides by name with `tests/run_tests.py`. **(11)** three `tests/` suites need the system interpreter for `lxml`. **(12)** the scan list needs tightening. **(13)** the companion-file rename is due. **(14)** `us/…/apply_translations_textmatch.py` line 937 carries the UK spelling `behaviour` — the reconciliation's to fix.

**What a new session would get wrong.** It would scope C17 from D08 and find nothing there. It would try to reproduce F16 from the corpus and read a clean zero as *already fixed*. It would point a test's apply at `tests/fixtures/` — apply writes `.validate-state.json` beside its NOTES file, and that reached the index once today. It would trust `audit_branches`' fixture claim as historical: it is now the only claim in that block reading the **index**, so a new fixture must be **STAGED** before the count is right. It would read `$?` after a pipe. It would trust a directory-level line-ending map — `uk/` and `us/` pure LF, `CLAUDE.md` and root `.md` pure CRLF, `tests/`, `tools/`, `.claude/` **MIXED per file**. It would run `verify_charter_continuity.py` without `LT_HOUSE_TEMPLATES`, whose check 7 then reports **VOID, not a pass**. **Three red lines are BY DESIGN:** `verify_md`'s `file length` while §1.7's exemption stands, `stepb_audit` check 10 at 8 unverified quotations, and `verify_code`'s commit-identity check, which is the decision above and not a defect to fix in passing.
