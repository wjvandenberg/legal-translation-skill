...
| **`STEP-B-ANALYSIS.md`** | **the build plan, and formally step 2's PLAN FILE** — adopted under the grandfather rule 2026-08-24, not renamed and not split. Four parts, and **only Part One is needed to build.** Its own header declares its size and its trigger | **before and during every build branch — it is the leading document for all of step 2** | its §2 the order · §3 the brief per option · §4 the test method · **section 9 of that document** the branch → findings map, **generated by script so it cannot drift** |
