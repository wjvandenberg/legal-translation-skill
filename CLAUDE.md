...
> **OVER-CAP EXEMPTION, RE-DECLARED 2026-09-02: 1,118 lines against a cap of 350.** *(Taken from the checker on
...
### HANDOFF — 2026-09-02. SLICE 2 IS MERGED. SLICE 3, THE PAGE-NUMBER WIDENING, IS BUILT AND NOT MERGED

**PR #59 merged as `049484e` on Wouter's page-by-page read, and both comparison pins moved to it in the same breath** — `apply_corpus_diff.py` and `render_diff.py`. A pin left behind reports the previous slice's work as the next branch's movement, and the prose above the first pin was itself stale, still naming `79a8c14` and `e57e658`. **Nothing checks a comment.**

**Produced: `feature/branch6-toc-widening`, not merged.** A table-of-contents page number may now be a **well-formed roman numeral in uniform case** (front matter) or **one-or-two letters + separator + digits** (`A-3`, schedules), beside the arabic digits already admitted. **Wouter's decision, taken at the input point** on the open question slice 2 left on the PR — it was declined by choice, never by defect.

**THE ACCEPTANCE CONDITION IS INVERTED FROM BOTH EARLIER SLICES, AND THAT IS THE POINT.** They had to move a delivered byte and explain every movement. This one must move **NONE**: no corpus document carries either form, so all thirteen frozen intermediates are the positive control. Measured **13 compared · 0 moved · 13 byte-quiet**, and the harness's *"byte-identical to baseline"* note was **absent**, so it was a real before-and-after rather than a file compared with itself. **18 of 18 shapes; 11 placed, 6 declined, control firing.**

**THE WIDENING SPENDS FALSE-POSITIVE MARGIN, SO IT WAS BOUGHT BACK WITH STRICTNESS.** The obvious roman pattern `[ivxlcdm]+` **matches the English word `civil`** — and `dill`, `lid`, `mill` — so the shipped rule is a **well-formedness** test plus uniform case, and four NEGATIVE shapes hold that line. One residue is declared, not hidden: `mix` is M + IX and *is* a well-formed numeral. Every vector is proved in `temp/probe_page_number.py`.

**AND THE ONLY RENDER THIS CHANGE CAN EVER HAVE IS SYNTHETIC, WHICH IS §5.7's THIRD ENTRY BITING.** `tests/fixtures/toc-widened.docx` was built for it and **looked at**: five entries gain a dot leader and a right-aligned number, the two traps render flat, nothing wraps. A count of 18 of 18 is not a page.

**THREE THINGS THE INSTRUMENTS CAUGHT IN THIS SESSION'S OWN WORK, NONE BY READING.**
- **`audit_branches` B1.fixtures was ALREADY RED at `049484e`** — 14 files against a claimed 13, because `toc.notes.json` landed and the number did not move. **The instrument's shape is why nobody saw it: the claim read `main`, so it was GREEN on the branch that broke it and red only after merge.** Now reads the **index**, so a fixture added on a branch is counted while it can still be fixed. Claim 16, counted by listing.
- **Apply writes `.validate-state.json` into whatever directory its NOTES file lives in**, via its final pre-apply `validate_translations` pass. The new fixture assertion pointed apply straight at `tests/fixtures/`, so run state reached the index — a dirty tree on a read-only check, `git bisect` broken, and one `git add -A` from committing it to a public repo. **Caller fixed (copy inputs to temp, as `render_diff` and `test_stop_deleting` always did), and the class guarded by path in `.gitignore`.**
- The pattern was proved against its vectors **before** it entered a shipped tree, and the separator class is built with `chr()` rather than typed: a literal en dash is invisible in review and one keystroke from becoming a hyphen.
...
**AND ONE OPEN ITEM IS NOT LIKE THE OTHERS, FOUND 2026-09-02 BY RUNNING `verify_code` AND TRACKED NOWHERE BEFORE THIS LINE.** Its `commit author identity` check FAILS: **105 of 198 identity slots across the whole history are not the GitHub noreply address, and 53 of those carry a NON-GITHUB DOMAIN.** *(Domains classified in aggregate; no address printed — the count is the finding.)* **This repository has been PUBLIC since 2026-08-07, and an author email is metadata on every commit, so it is the one thing a content scan can never see** — every CLEAN confidentiality report this project has ever produced was correct and blind to it. **This session's own commits are clean on both author and committer, measured.** The house rule says *always rewrite BEFORE a repo goes public, never after*, and that window has closed; `verify_code`'s own message adds that a rewrite after a repo is public and cloned does not reliably unserve it. **`author_allow_before` exists to grandfather exactly this and keep the accepted count visible rather than silent — but setting it ACCEPTS a published exposure, which is Wouter's decision and belongs in §3 with a trigger, not in a checker config changed in passing.**
