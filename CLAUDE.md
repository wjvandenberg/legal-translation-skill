...
> **OVER-CAP EXEMPTION, RE-DECLARED 2026-09-08: 1,167 lines against a cap of 350.** *(Taken from the checker on
...
### HANDOFF — 2026-09-08. BRANCH 7 SLICE 1 MERGED. NEXT IS SLICE 2, THE GLOSSARY ROUTE

**Slice 1 merged as `544f908`, PR #65. A16 and N1 CLOSED.** One explicit inventory of run-bearing containers, **stated in FOUR files — extract and apply, in each tree — with their equality ASSERTED**, because the 2026-08-05 decision rules out a shared library and "shared" is otherwise unenforceable. **Both comparison pins moved to `544f908` as the first act after the merge** — derived from `git log --oneline -1 -- uk us` with `git diff` empty, never read off the merge message. **The pin has moved five times in four days.** **CLOSED AT 67% OF CONTEXT** — read from the screen by Wouter, never estimated, and the fourteenth reading in the series: 32, 41, 48, 29, 38, 26, 41, 43, 32, 32, 47, 54, 46, 67. **It is the HIGHEST yet and it broke the band a second time**, on a session that ran one whole slice plus its close. Recorded, never obeyed — the session ended because its planned work ran out.

**THE CLASS IS TWELVE, NOT THREE.** The rule *stop listing containers* was written from the three instances anybody had noticed. Measured through the real apply over **20 shapes, every expectation written down first**: `sdt`, `smartTag`, `customXml`, `dir` and `bdo` all strand text identically; **`customXml`, `dir` and `bdo` are named in no register row.** 19 of 20 matched the prediction and the miss was worse than predicted. **And the second consequence is not the remnant:** where the container held the paragraph's FIRST text the English was placed AFTER it, so the delivered reading order was wrong before anything was deleted.

**ACCEPTANCE: 13 documents · 2 moved · 11 byte-identical · 0 unexplained · exit 0**, freeing 1 stranded fragment on the sdt document and its batch arm. **Seen on the synthetic fixture, all three arms.** `sdt` and `smartTag` left the harness's `HOLD` class — and NOT for `RESTORE`, which a fix that did nothing would satisfy: the count must hold while the contents change.

**A16's ROW WAS RIGHT WHERE THIS SESSION FIRST SAID IT WAS WRONG.** Its `en == text` sentence was refuted from XML shape, then re-measured against the NOTES: idx 80 and 82 have `en == text` and are skipped — the row's mechanism verbatim and its count of two — and idx 12 is the single instance the fix moves. **§1.5 gains that as its third reading rule.** **N1 closes on the fixture alone, declared:** the pristine document carries the one corpus smart tag and the copy the pipeline applied to does not, so it went upstream in a conversion — **§5.7's fourth reason a corpus cannot reach a defect.**

**THE SINGLE NEXT ACTION: slice 2 — C19, the glossary route.** Add `--glossary` to `repack_docx.py` **plus the refusal repack already applies to numbering, comments, footnotes and endnotes** — its own convention, not a new mechanism, though note the trigger differs: theirs fire on a workdir file looking translated, this must fire on the ORIGINAL carrying the part. Also Step 8's inventory in both trees. **Proved end to end: the glossary part comes back byte-identical, and extraction never reads it either, so the gap is on BOTH sides** — the row frames it as repack's alone. Exactly one corpus document carries the part. **Slice 2's acceptance condition is NOT slice 1's:** `apply_corpus_diff` drives apply, not repack, so it should report 13 byte-identical and that all-quiet result proves nothing about slice 2. The fixture is the acceptance. **Then slice 3 — A19:** report every graphic surface and **translate the alt text** *(§2.6, 2026-09-08)*. The corpus has 14 drawings, all in headers/footers, and **not one `@descr`**, so the translation is proved on a fixture alone and that must be said.

**Open, none blocking.** **(1)** slices 2 and 3, above. **(2) TWO MEASUREMENTS NOW OWED, both scheduled at the close rather than left as prose** — the **header/footer `sdt` surface** (5 across 4 documents; `translate_headers_footers.py` appears to strand nothing because `_iter_own_runs` writes into the first `w:t` and clears the rest, **read but NOT RUN**), and the **`w:dataBinding` question** (2 documents carry a bound control, and Word can repopulate its text from `customXml` on open, so a correct `document.xml` edit may be undone on the page — needs Word, which no instrument has). **(3)** **A22 is a DEADLOCK, not a duplication** — `validate_apply --strict` refuses the repack — and it gets no branch yet, because fixing it reopens clause 3's keyword narrowing, itself set on a corpus measurement; zero corpus instances, so nothing ships wrong. **(4)** whether the upstream removal of a smart tag is CORRECT: defensible, undocumented, and if the conversion is rescoped the defect returns with nothing able to see it. **(5)** everything branch 6 left: C16's residue, A20's corpus measurement, the commit-identity decision (**105 of 198 identity slots**, 53 non-GitHub, a §3 decision with a trigger), G12's 16 unclassified findings, the scan list's false-positive rate, `verify_code`'s other two reds.

**What a new session would get wrong.** It would trust a fixture built from a register row's TITLE — **this one carried the shape the row calls correct.** It would refute a behavioural claim from structure. It would read a sweep's pass rate as coverage; **20 of 20 is a statement about a list somebody wrote.** It would confuse `LOST` with `DROPPED` — a dropped annotation wrapper is the decided outcome, a destroyed ruby is not. It would include `w:hyperlink` in a container count, which branch 6 already fixed, producing a false *"the fix did not fire"*. It would add a new harness arm without wiring it into the older arm's verdict, and read its own branch's work as unexplained. **It would forget that `FIX_LANDED` needs a row the moment a directional check's fix merges** — that table is why the harness now reports **zero** on a self-comparison where it once reported four guaranteed defects. It would run `tests/make_fixtures.py` with bare `python` and rewrite all sixteen fixtures' bytes silently *(I-25: zlib-ng versus stock zlib)*. It would point a test's apply at `tests/fixtures/`, or forget to **stage** a new fixture before `audit_branches` counts it. **It would run `verify_charter_continuity.py` with `LT_HOUSE_TEMPLATES` set to `…\Coding\templates`, where check 7 FAILS** — it must point at **`…\Coding\templates\evidence`**; the previous handoff had this wrong. **Two reds are BY DESIGN** — `verify_md`'s `file length` while §1.7's exemption stands, and `stepb_audit` check 10 at 8 unverified quotations. **`verify_code` is a third and fails three checks.**
