...
> **OVER-CAP EXEMPTION, RE-DECLARED 2026-09-08: 1,173 lines against a cap of 350.** *(Taken from the checker on
...
### HANDOFF — 2026-09-08. BRANCH 7 SLICE 1 BUILT, NOT MERGED. A16 AND N1 CLOSED; TWO SLICES REMAIN

**Branch `feature/container-inventory`. Slice 1 of three, and slices 2 and 3 are agreed and not started.** **A16 and N1 CLOSED.** Apply now recurses into every listed container, so a content control's source text is removed with the rest of the paragraph's and the English lands in reading order. **The inventory is stated in FOUR files — extract and apply, in each tree — and their equality is asserted**, because the 2026-08-05 decision rules out a shared library and "shared" is otherwise unenforceable. **The asymmetry WAS the defect, not either half.**

**THE CLASS IS TWELVE, NOT THREE, AND THAT IS THE HEADLINE.** The rule *stop listing containers* was written from three instances anybody had noticed. Measured through the real apply over **20 shapes, every expectation written down first**: `sdt`, `smartTag`, `customXml`, `dir` and `bdo` all strand text identically. **19 of 20 matched the prediction; the miss was worse than predicted.** Now **17 CLEAN · 1 DROPPED · 1 LOST · 1 REMNANT**, the last two PINNED.

**TWO NEW ROWS, NEITHER BRANCH 7's TO FIX, both pinned in `tests/test_container_inventory.py`.** **A21** — a `w:ruby` annotation **destroyed outright**, ruby/rubyBase/rt 1→0, with **no text lost**, because extraction had already glued the reading to its base; putting it back needs branch 15's per-run English. **A22** — a `w:fldSimple` prints its cached number **twice**: A9's defect in the one field form clause 3 cannot see, and widening clause 3 reopens a corpus measurement.

**ACCEPTANCE: 13 documents · 2 moved · 11 byte-identical · 0 unexplained · exit 0.** The container arm freed **1** stranded fragment on the sdt document and on its batch arm, 5 → 4 each.

**AND A16's ROW WAS RIGHT WHERE I SAID IT WAS WRONG.** Its `en == text` sentence was refuted from XML shape, written into the plan, then re-measured against the **notes**: idx 80 and 82 have `en == text` and are skipped — the row's mechanism verbatim and its count of two — and idx 12 is the single instance the fix moves. §1.5 gains that as its third reading rule: **a claim about behaviour is measured where the behaviour happens.**

**N1 CLOSES ON THE FIXTURE ALONE, AND IT IS DECLARED.** The pristine document carries the one corpus smart tag; **the copy the pipeline applied to does not**, while that paragraph's notes exist and `en != text`. Nothing in either tree removes one, so it went upstream in a conversion. **§5.7 gains a fourth reason a corpus cannot reach a defect.**

**FIVE INSTRUMENT DEFECTS, ALL FIXED THIS SESSION, FOUR IN THIS SESSION'S OWN PROBE.** A second copy of the container list that omitted `ruby`, so a **deleted** container read CLEAN · a sentinel that cannot survive the tracked-change path splitting one string · a substring test where a surviving `7` matched the token `z7` · an order check that flagged the two cases it should pass. **And two committed:** **I-23**, `make_fixtures` reassigning `sys.stdout` on import and closing the shared buffer for any importer — 4 test modules and 19 tools wrap stdout, so it was a collision waiting for the next importer; **I-24**, the acceptance harness reporting defects it was **guaranteed** to report, three ways. **§5.16 gains a fourth rule with no house twin.**

**THE SINGLE NEXT ACTION: get slice 1 reviewed and merged, then build slice 2 — C19's glossary route.** Add `--glossary` to `repack_docx.py` plus the refusal repack already uses for its five other aux parts, and Step 8's inventory in both trees. **Proved end to end: the glossary part comes back byte-identical, and extraction never reads it either, so the gap is on BOTH sides.** Exactly one corpus document carries the part. **Then slice 3 — A19:** report every graphic-metadata surface and **translate the alt text** (Wouter, 2026-09-08 — a `@descr` is what a screen reader speaks). The corpus has 14 drawings, all in headers/footers, and **not one `@descr`**, so translation is proved on the fixture and that must be said.

**Open, none blocking.** **(1)** slices 2 and 3, above. **(2)** whether the upstream removal of a smart tag is CORRECT — defensible, undocumented, and if the conversion is ever rescoped the defect returns with nothing able to see it. **(3)** A22 needs a decision on clause 3's keywords before it can be fixed. **(4)** the header/footer surface: **5 `w:sdt` across 4 documents**, and `translate_headers_footers.py` handles them correctly by a different mechanism (`_iter_own_runs` writes into the first `w:t` and clears the rest) — measured by reading, **not yet run**, so it is a claim rather than a result. **(5)** `w:dataBinding` on 2 corpus documents: Word can repopulate a bound control from `customXml` on open, so a correct `document.xml` edit may be undone on the page. Needs Word; unmeasured. **(6)** everything branch 6 left: C16's residue, A20's corpus measurement, the commit-identity decision (**105 of 198 identity slots**, 53 non-GitHub, a §3 decision with a trigger), G12's 16 unclassified findings, the scan list's false-positive rate, `verify_code`'s other two reds.

**What a new session would get wrong.** It would trust that a fixture built from a register row's TITLE carries that row's defect — **this one carried the shape the row calls correct**. It would refute a behavioural claim from structure. It would read a sweep's pass rate as coverage; **20 of 20 is a statement about a list somebody wrote**. It would take `LOST` and `DROPPED` to mean the same thing — a dropped annotation wrapper is the decided outcome, a destroyed ruby is not. It would put `sdt`/`smartTag` in the harness's `RESTORE` class, where a fix that did nothing would pass: the counts must **hold** while the contents change. It would include `w:hyperlink` in the container count, which was already fixed and produces a false "the fix did not fire". It would leave a new arm out of the older arm's verdict and read its own branch's work as unexplained. It would point a test's apply at `tests/fixtures/`. It would forget to **stage** a new fixture before `audit_branches` counts it. It would run `verify_charter_continuity.py` without `LT_HOUSE_TEMPLATES`, whose check 7 then reports **VOID, not a pass**. **Two reds are BY DESIGN** — `verify_md`'s `file length` while §1.7's exemption stands, and `stepb_audit` check 10 at 8 unverified quotations. **`verify_code` is a third red, failing three checks.**
